...
void Application::ToggleChatState() {
    ...
    if (device_state_ == kDeviceStateIdle) {
        ...
        Schedule([this]() {
            ...
            ESP_LOGI(TAG, "停止持续监听模式");
            SetDeviceState(kDeviceStateIdle);
        });
    }
    // 移除 kDeviceStateSpeaking 状态处理，因为不再有语音输出
}
...
void Application::StartListening() {
    ...
    // 移除对 kDeviceStateSpeaking 状态的处理
}

void Application::StopListening() {
    Schedule([this]() {
        if (device_state_ == kDeviceStateListening) {
            keep_listening_ = false; // 明确停止持续监听
            protocol_->SendStopListening();
            protocol_->CloseAudioChannel();
            SetDeviceState(kDeviceStateIdle);
        }
...
void Application::Start() {
    ...
    protocol_->OnIncomingJson([this, display](const cJSON* root) {
        ...
        if (strcmp(type->valuestring, "stt") == 0) {
            ...
            if (text != NULL) {
                ESP_LOGI(TAG, ">> [持续监听模式] 语音识别: %s", text->valuestring);
                Schedule([this, display, message = std::string(text->valuestring)]() {
                    display->SetChatMessage("user", message.c_str());
                    
                    // 创建后台任务来处理延迟清理
                    background_task_->Schedule([this, display]() {
                        vTaskDelay(pdMS_TO_TICKS(3000)); // 显示3秒
                        Schedule([this, display]() {
                            display->SetChatMessage("user", ""); // 清空用户消息
                            // 保持持续监听，不关闭音频通道
                            ESP_LOGI(TAG, "继续持续监听模式...");
                        });
                    });
                });
            }
    ...
    xTaskCreate([](void* arg) {
        Application* app = (Application*)arg;
        vTaskDelay(pdMS_TO_TICKS(3000)); // 等待3秒让系统完全启动
        app->Schedule([app]() {
            if (app->GetDeviceState() == kDeviceStateIdle) {
                ESP_LOGI(TAG, "Auto starting continuous listening mode...");
                app->ToggleChatState(); // 启动持续监听
            }
        });
        vTaskDelete(NULL);
    }, "auto_listen", 2048, this, 1, nullptr);
}
...
void Application::AbortSpeaking(AbortReason reason) {
    // 不再需要中止语音播放，因为已经移除了语音输出功能
    ESP_LOGI(TAG, "AbortSpeaking called but speech output is disabled");
}
...
void Application::SetDeviceState(DeviceState state) {
    ...
    switch (state) {
        ...
        case kDeviceStateListening:
            if (keep_listening_) {
                display->SetStatus("持续监听"); // 显示持续监听状态
            } else {
...
void Application::WakeWordInvoke(const std::string& wake_word) {
    ...
    // 移除对 kDeviceStateSpeaking 状态的处理，因为不再有语音输出
}
